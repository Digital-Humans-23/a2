...
public:
    ...
    const fs::path &GetPwd() const noexcept;
    ...
    std::vector<fs::path> GetMultiSelected() const;
...
private:
    ...
    fs::path pwd_;
    std::set<fs::path> selectedFilenames_;
        ...
        reserveHeight += GetFrameHeightWithSpacing();
    {
        ...
        for (auto &rsc : fileRecords_) {
            ...
            if (Selectable(rsc.showName.c_str(), selected, ImGuiSelectableFlags_DontClosePopups)) {
                const bool multiSelect = (flags_ & ImGuiFileBrowserFlags_MultipleSelection) && IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) &&
...
inline const class fs::path &ImGui::FileBrowser::GetPwd() const noexcept {
    return pwd_;
}
...
inline std::vector<fs::path> ImGui::FileBrowser::GetMultiSelected() const {
    if (selectedFilenames_.empty())
        return {pwd_};
...
inline void ImGui::FileBrowser::SetPwdUncatched(const fs::path &pwd) {
    ...
    std::sort(fileRecords_.begin(), fileRecords_.end(),
              [](const FileRecord &L, const FileRecord &R) { return (L.isDir ^ R.isDir) ? L.isDir : (L.name < R.name); });
