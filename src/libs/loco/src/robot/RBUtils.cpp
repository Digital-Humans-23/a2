...
KeyWord RRBkeywords[] = {{"RB", RB_RB},
                         ...
                         {"collisionBox", RB_COLLISION_BOX},
                         {"collisionCylinder", RB_COLLISION_CYLINDER},
                         {"collisionCapsule", RB_COLLISION_CAPSULE},
                         ...
                         {"jointMaxSpeed", RB_JOINT_MAX_SPEED},
                         {"jointMaxTorque", RB_JOINT_MAX_TORQUE},
...
int getRRBLineType(char *&buffer) {
    return getLineType(buffer, RRBkeywords, sizeof(RRBkeywords) / sizeof(RRBkeywords[0]));
}

char *getRRBString(int token) {
    return getKeyword(token, RRBkeywords, sizeof(RRBkeywords) / sizeof(RRBkeywords[0]));
}
