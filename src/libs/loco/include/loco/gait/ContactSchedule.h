#pragma once

#include "loco/LeggedRobot.h"
#include "crl-basic/utils/json_helpers.h"
#include "crl-basic/utils/trajectory.h"
#include "crl-basic/utils/utils.h"

using namespace std;

namespace crl::loco {

class SwingPhaseContainer {
public:
    // this is the limb the swing phase is applied to...
    std::string limbName;

    // keep track of when a contact should be broken (e.g. a foot lifts), and when it should be established again - the swing phase is defined to take place
    // between these two discrete events...
    DynamicArray<pair<double, double>> swingPhases;

public:
    explicit SwingPhaseContainer(const std::string &limbName) : limbName(limbName) {}

    void addSwingPhase(double start, double end) {
        // the swing phase must end after it begins!
        if (start >= end) {
            assert(false);
            return;
        }
        swingPhases.emplace_back(start, end);
    }

    void clearSwingPhasesBefore(double t) {
        while (!swingPhases.empty() && swingPhases.front().second < t)
            swingPhases.erase(swingPhases.begin());
    }
};

class PeriodicGait {
public:
    // this is the duration of the stride, in seconds...
    double strideDuration = 1.0;

    // and the swing period is represented here relative to the overall phase of
    // the gait cycle... e.g. starts at 0.25 and ends at 0.4, where the gait
    // cycle is normalized to go from 0 to 1.
    DynamicArray<SwingPhaseContainer> swingPhases;

public:
    PeriodicGait() = default;

    void addSwingPhaseForLimb(const std::string &limbName, double start, double end) {
        // the following must hold for a swing phase
        //	1) it must start before it ends
        //	2) the interval (start, end) must have a non-zero intersection
        // with (0, 1), so either end is greater than 0 (and start can be either
        // positive or negative), or start is less than 1 (and end can be less
        // than or greater than 1) 	3) end-start must be less than or equal
        // to 1 ( if equal to 1 this means the limb is always in swing)
        if (!(start < end)) {
            assert(false);
            return;
        }
        if (!(end > 0 || start < 1)) {
            assert(false);
            return;
        }
        if (!(end - start)) {
            assert(false);
            return;
        }

        swingPhases.emplace_back(limbName);
        swingPhases.back().addSwingPhase(start, end);
    }
};

#define CP_FOREVER_AND_EVER 1000

class ContactPhaseInfo {
private:
    // if inSwing is false, it must be in stance mode
    bool swingMode = false;
    // this is the total length of time for the current swing or stance phase...
    double contactPhaseDuration = 0;
    // this is the amount of time left until this contact phase ends and the
    // next begins...
    double timeLeft = 0;

public:
    /**
     * at any moment in time, each limb is either in swing (the period of time
     * during which there is no contact) mode or in stance (the phase during
     * which there should be a contact with the environment) mode
     */
    ContactPhaseInfo(bool swingMode, double contactPhaseDuration, double timeLeft) {
        if (contactPhaseDuration < 0.0001)
            contactPhaseDuration = 0.0001;
        boundToRange(timeLeft, 0, contactPhaseDuration);
        this->swingMode = swingMode;
        this->contactPhaseDuration = contactPhaseDuration;
        this->timeLeft = timeLeft;
    }

    //default constructor assumes a limb is forever in stance mode...
    ContactPhaseInfo() {
        this->swingMode = false;
        this->contactPhaseDuration = CP_FOREVER_AND_EVER;
        this->timeLeft = CP_FOREVER_AND_EVER;
    }

    bool isSwing() const {
        return swingMode;
    }

    bool isStance() const {
        return !swingMode;
    }

    /**
     * time left until the next phase starts
     */
    double getTimeLeft() const {
        return timeLeft;
    }

    double getTimeElapsed() const {
        return contactPhaseDuration - timeLeft;
    }

    double getDuration() const {
        return contactPhaseDuration;
    }

    /**
     * relative amount of elapsed time in this swing/stance mode. 0 means it is
     * at the very start, 1 means it is at the end
     */
    double getPercentageOfTimeElapsed() const {
        return (contactPhaseDuration - timeLeft) / contactPhaseDuration;
    }
};

/**
 * keep track of the desired pattern of planned contact phases for all limbs.
 * Note this class is aka a footfallpattern, if we're talking pure legged
 * locomotion
 */
class ContactSchedule {
public:
    // swing phases stored in absolute time...
    DynamicArray<SwingPhaseContainer> swings;

public:
    void addSwingPhaseForLimb(const std::string &limbName, double start, double end) {
        if (!(start < end)) {
            assert(false);
            return;
        }
        if (!(end > 0 || start < 1)) {
            assert(false);
            return;
        }
        if (!(end - start)) {
            assert(false);
            return;
        }

        int lIndex = 0;

        // periodic gaits will admit only one swing phase per gait cycle...
        for (auto ls : swings) {
            if (ls.limbName == limbName)
                break;
            lIndex++;
        }

        if (lIndex == (int)swings.size())
            swings.emplace_back(limbName);

        // now, make sure that this new swing phase starts after the previous
        // one has ended...
        if (swings[lIndex].swingPhases.size() > 0) {
            // if the new swing phase starts right when the previous one ends,
            // merge them...
            if (fabs(swings[lIndex].swingPhases.back().second - start) < 0.0001) {
                swings[lIndex].swingPhases.back().second = end;
                return;
            }
            if (swings[lIndex].swingPhases.back().second > start) {
                assert(false);
                return;
            }
        }

        swings[lIndex].addSwingPhase(start, end);
    }

    void addPeriodicGaitToContactSequence(const PeriodicGait &pg, double startTime) {
        for (auto ls : pg.swingPhases)
            addSwingPhaseForLimb(ls.limbName, startTime + ls.swingPhases.front().first * pg.strideDuration,
                                 startTime + ls.swingPhases.front().second * pg.strideDuration);
    }

    /**
     * returns a data structure that can be used to determine, based on absolute
     * time t, if the limb is in swing or stance mode, time remaining for the
     * current contact configuration, etc...
     */
    ContactPhaseInfo getContactPhaseInformation(const std::string &limbName, double t) const {
        const SwingPhaseContainer *spc = nullptr;
        for (const auto &swing : swings)
            if (swing.limbName == limbName) {
                spc = &swing;
                break;
            }

        // if we have no information for this limb, we assume no one gave it any
        // swing phases, so it must be allways in stance then...
        if (spc == nullptr)
            return {false, CP_FOREVER_AND_EVER, CP_FOREVER_AND_EVER};

        // if this limb has been registered but it has no swing phases after t,
        // then it should be assumed it will remain in stance for the forseable
        // future...
        if (spc->swingPhases.empty())
            return {false, CP_FOREVER_AND_EVER, CP_FOREVER_AND_EVER};

        // now, t might be before any of the swing phases are starting...
        if (spc->swingPhases.front().first > t)
            return {false, spc->swingPhases[0].first - t, spc->swingPhases[0].first - t};

        // or it could be after the last swing phase ends...
        if (spc->swingPhases.back().second < t)
            return {false, CP_FOREVER_AND_EVER, CP_FOREVER_AND_EVER};

        // if none of the above are true, t must fall between swing phases, or
        // within a swing phase...
        for (int i = 0; i < (int)spc->swingPhases.size(); i++) {
            // see if t falls within a swing phase
            if (t >= spc->swingPhases[i].first && t <= spc->swingPhases[i].second)
                return {true, spc->swingPhases[i].second - spc->swingPhases[i].first, spc->swingPhases[i].second - t};
            // since swing phases are sorted, if t is smaller than the next
            // swing phase, it means it falls within a contact region... note:
            // we know that t does not start after the last swing phase, so the
            // "i+1" here should be safe...
            if (t <= spc->swingPhases[i + 1].first)
                return {false, spc->swingPhases[i + 1].first - spc->swingPhases[i].second, spc->swingPhases[i + 1].first - t};
        }

        // all right, we should never get here...
        assert(false);
        // but the show must go on...
        return {false, CP_FOREVER_AND_EVER, CP_FOREVER_AND_EVER};
    }
};

class ContactPlanManager {
public:
    DynamicArray<pair<double, double>> strideUpdates;

    // this is the time when the last periodic ffp was added. When a new one will be added, it will go from there...
    double timeStampForLastUpdate = 0;
    // visualization options here...
    double visTimeWindow = 3;
    // this is the overall window width we want for the visualizer...
    double cpmWindowWidth = 1300;
    double labelWindowWidth = 50;
    mutable bool drawLabels = true;

    // this is the schedule of planned swing and stance phases for all the limbs...
    ContactSchedule cs;

public:
    ContactPlanManager() = default;

    void appendPeriodicGaitToPlanningHorizon(const PeriodicGait &pg) {
        cs.addPeriodicGaitToContactSequence(pg, timeStampForLastUpdate);
        strideUpdates.emplace_back(timeStampForLastUpdate, timeStampForLastUpdate + pg.strideDuration);
        timeStampForLastUpdate += pg.strideDuration;
    }

    void appendSwingPhases(const std::map<std::string, std::vector<std::pair<double, double>>> &swingPhases) {
        // TODO: how to do stride updates?
        for (const auto &map : swingPhases) {
            for (const auto &swing : map.second) {
                cs.addSwingPhaseForLimb(map.first, swing.first, swing.second);
                if (swing.second > timeStampForLastUpdate)
                    timeStampForLastUpdate = swing.second;
            }
        }
    }

    double timeUntilEndOfPlanningHorizon(double t) const {
        return timeStampForLastUpdate - t;
    }

    void tidyUp(double t) {
        for (uint i = 0; i < cs.swings.size(); i++)
            cs.swings[i].clearSwingPhasesBefore(t);
        while (strideUpdates.size() > 0 && strideUpdates.front().second < t)
            strideUpdates.erase(strideUpdates.begin());
    }

    ContactPhaseInfo getCPInformationFor(const std::string &limbName, double t) const {
        return cs.getContactPhaseInformation(limbName, t);
    }

    const SwingPhaseContainer *getSwingPhaseContainerForLimb(const std::string &limbName) const {
        for (const auto &swing : cs.swings)
            if (swing.limbName == limbName)
                return &swing;
        return nullptr;
    }

    float getWindowCoord(double tVal, double tStart, double tEnd) const {
        double p = (tVal - tStart) / (tEnd - tStart);
        return labelWindowWidth + (float)p * (cpmWindowWidth);
    }

    void visualizeContactSchedule(double t, double gridStartTime = -1, double dt_grid = 1 / 30.0, int nGridSteps = 30) const;
};

}  // namespace crl::loco
