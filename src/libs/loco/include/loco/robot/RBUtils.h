...
enum RB_KEYWORDS {
    ...
    RB_COLLISION_SPHERE,
    RB_COLLISION_BOX,
    RB_COLLISION_CYLINDER,
    RB_COLLISION_CAPSULE,
    RB_COLLISION_PLANE,
    RB_MESH_NAME,
    RB_MESH_COLOR,
...
inline double getHeadingAngle(const Quaternion &q) {
    ...
    computeEulerAnglesFromQuaternion(q, V3D(1, 0, 0), V3D(0, 0, 1), V3D(0, 1, 0), roll, pitch, yaw);
    return yaw;
}
...
inline void testQuaternionDecomposition() {
    for (int i = 0; i < 100; i++) {
        ...
        Quaternion qErr = getRotationQuaternion(bValTest, b) * getRotationQuaternion(aValTest, a) * qTest.inverse();
